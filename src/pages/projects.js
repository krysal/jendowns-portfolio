...
export default () => {
  const renderProjects = data =>
    data.projects.edges.map(project => {
      ...
      return (
        <article
          className="container--projects__project"
          key={project.node.childMarkdownRemark.frontmatter.path}
        >
          <BgImage
          ...
          <a
            ...
            href={project.node.childMarkdownRemark.frontmatter.path}
          >
            <h3 className="container--projects__project__title">
              <span>{project.node.childMarkdownRemark.frontmatter.title}</span>
            </h3>
          </a>
        </article>
      )
        ...
        query Projects {
          ...
          cah: imageSharp(id: { regex: "/cah/" }) {
            sizes(
              maxWidth: 900
              quality: 100
              duotone: { highlight: "#edfafa", shadow: "#4a00e0" }
            ) {
